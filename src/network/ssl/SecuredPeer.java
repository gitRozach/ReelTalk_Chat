...
//		                    return false;
    ...
    protected void putDataIntoBufferAndFlip(byte[] data, boolean clearBufferBeforeAdding) {
	...
	protected void writeEncryptedData(SocketChannel clientChannel) throws IOException {
		myNetData.flip();
		while (myNetData.hasRemaining())
			clientChannel.write(myNetData);
	}

	protected SSLEngineResult encryptBufferedData(SSLEngine engine) {
		try {
			myNetData.clear();
